...
app.post("/auth/login", async (req, res) => {
  ...
  let pass = '1234'
  let data = {}
  let loginuser = `SELECT * FROM twitter_users WHERE username = '${username}' OR email = '${username}' OR phone = '${username}' AND password = ${pass}`
  try {
...
// create tweets api 
app.post("/user/tweets/:user_id", verifyToken, async (req, res) => {
  let user_id = req.params.user_id
  let tweet_text = 'hello'
  let data = {}
  // let i = req.files.file
  // let img = i.mv(`/public/documents/${i}`)
  let createtweets = `INSERT INTO twitter_tweets (user_id,tweet_text,image) VALUES ` + `
  (${user_id},'${tweet_text}','img')`
  try {
    let result = await performQuery(createtweets)
    data.tweet_id = result.insertId;
    data.status = true;
    res.send(data)
  } catch (error) {
...
app.post("/user/editProfile/:user_id", verifyToken, async (req, res) => {
  let user_id = req.params.user_id
  let name = 'yug'
  let username = 'yug2611'
  let pass = '12345'
  let city = 'ahmedabad'
  let state = 'gujarat'
  let country = 'india'
  ...
  let data = {}
  // res.send(req.user)
  let updatedata = `UPDATE twitter_users SET name = '${name}',username = '${username}',password = '${pass}',city = '${city}',state = '${state}',country = '${country}',dob = '${dob}' WHERE user_id = ${user_id}`
  try {
    await performQuery(updatedata)
    // data.user_id = result.insertId;
    data.status = true;
    res.send(data)
  } catch (error) {
